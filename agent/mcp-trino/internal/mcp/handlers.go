...
// GetStockByCodition handles return stock have highest price which have event "Trả cổ tức bằng tiền mặt"
func (h *TrinoHandlers) GetStockByCodition(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
    if h.Config.EnableImpersonation {
        ctx = h.prepareImpersonationContext(ctx)
    }
	...
	if !ok {
		mcpErr := fmt.Errorf("event parameter must be a string")
		return mcp.NewToolResultErrorFromErr(mcpErr.Error(), mcpErr), nil
	}
	// limit, ok := args["limit"].(float64)
	// if !ok {
	// 	mcpErr := fmt.Errorf("limit parameter must be a number")
	// 	return mcp.NewToolResultErrorFromErr(mcpErr.Error(), mcpErr), nil
	// }
	limit := 10

	// Execute the query - SQL injection protection is handled within the client
	results, err := h.TrinoClient.GetStock(event, limit)
	if err != nil {
		log.Printf("Error executing query: %v", err)
		mcpErr := fmt.Errorf("query execution failed: %w", err)
		return mcp.NewToolResultErrorFromErr(mcpErr.Error(), mcpErr), nil
...
func RegisterTrinoTools(m *server.MCPServer, h *TrinoHandlers) {
	...
	m.AddTool(mcp.NewTool("execute_query",
		...
		mcp.WithDescription("Execute SQL queries on Trino's fast distributed query engine for big data analytics. By default, only read-only queries (SELECT) are allowed for security. When TRINO_ALLOW_WRITE_QUERIES=true is set, supports all SQL statements including INSERT, UPDATE, DELETE, CREATE, DROP, and other DML/DDL operations. Perfect for complex analytics, aggregations, joins, and cross-system data exploration on large datasets."),
	...
	// m.AddTool(mcp.NewTool("get_stock_by_codition",
	// 	mcp.WithDescription("Get stock infor math with given event"),
	// 	mcp.WithTitleAnnotation("Get Stock By Codition"),
	// 	mcp.WithDestructiveHintAnnotation(true),
	// 	mcp.WithString("event", mcp.Required(), mcp.Description("Given event to query")),
	// 	mcp.WithNumber("limit", mcp.Description("Restrict the number of rows returned for trino query (optional)")),
	// ), h.GetStockByCodition)
}
